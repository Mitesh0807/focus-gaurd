...
async function checkPermissions() {
  ...
  const required = {
    permissions: ['storage', 'declarativeNetRequest', 'declarativeNetRequestFeedback', 'tabs', 'alarms', 'notifications', 'contextMenus'],
    origins: ['<all_urls>']
  };

  const hasAllPermissions = required.permissions.every(p => permissions.permissions.includes(p));
  const hasAllOrigins = required.origins.every(o => permissions.origins.includes(o));
  ...
  output.textContent = JSON.stringify({
    ...
    hasAllPermissions,
    hasAllOrigins,
    status: (hasAllPermissions && hasAllOrigins) ? '✅ All permissions granted' : '❌ Missing permissions'
  }, null, 2);
}
...
async function loadActiveRules() {
  ...
  try {
    ...
    if (rules.length === 0) {
      output.textContent = '⚠️ No active rules found!\n\nThis means blocking is NOT working.';
      return;
    }
...
function checkExtensionStatus() {
  ...
  chrome.runtime.sendMessage({ action: 'ping' }, (response) => {
    if (chrome.runtime.lastError) {
      ...
      messages.push(`<div class="status success">✅ Background script is running</div>`);
    }
...
async function fixBlocking() {
  ...
  try {
    ...
    if (response && response.success) {
      ...
      status.innerHTML += '<div class="status info">⚠️ Response received but unclear status</div>';
    }
