...
chrome.runtime.onInstalled.addListener(async (details) => {
  ...
  if (details.reason === 'install') {
    await chrome.storage.local.set({
      blockedSites: [],
      settings: { workMode: false, focusMode: false, notifications: true, darkMode: false },
      categories: getDefaultCategories(),
      stats: { blocksToday: 0, totalBlocks: 0, lastResetDate: new Date().toDateString() }
    });
  }
...
async function updateBlockList() {
  ...
  console.log('FocusGuard: Blocking', blockedUrls.length, 'sites:', blockedUrls);
